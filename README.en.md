...
The DB schema is versioned with [Alembic](https://alembic.sqlalchemy.org/) (`alembic.ini`,
`alembic/versions/`). `alembic upgrade head` runs automatically:

- on every start of the `web` container (`backend/main.py`'s `lifespan()` →
  `backend/db_migrate.py`) — the app doesn't start accepting requests until the schema is
  up to date;
- explicitly, a second time, during `sudo js` (`install.sh`) — as a visible, separate step
  in the update log with a clear error if something goes wrong.

Both calls are idempotent — running `alembic upgrade head` again when the schema is already
current is a no-op. `backend/db_migrate.py` also handles one-time "adoption" of a DB that
already has tables but has never run Alembic (`alembic stamp head` instead of `upgrade head`
— just a row in the bookkeeping `alembic_version` table, no DDL at all).

Locally, generate a new migration for a model change (`backend/models.py`) like this:

```bash
export DATABASE_URL=sqlite+aiosqlite:///path/to/test.db
alembic upgrade head                                   # apply everything that already exists
alembic revision --autogenerate -m "short description" # generate the new one
alembic upgrade head                                   # apply it and confirm it doesn't fail
alembic revision --autogenerate -m "should be empty"    # sanity check: diff must come up empty
```
Always open the autogenerated file and review it by hand — autogenerate doesn't see column
renames (it sees a DROP+ADD instead) and sometimes proposes operations you don't want.

---

## Backups

Manual control lives under the **Backups** tab in the admin panel (`superadmin` only): list/
download existing backups and a "Create now" button (`GET/POST /api/admin/backups*`,
`GET /api/admin/backup` — see `backend/routers/admin_system.py`).

An automatic backup is created every day at midnight UTC (`_auto_backup_task` in
`backend/main.py`) into `BACKUP_DIR` (defaults to `/data/backups`, the `db_data` volume) —
the last 7 backups are kept, older ones are deleted.

**Offsite copy (optional).** If the `BACKUP_REMOTE_PATH` environment variable is set, every
daily automatic backup is additionally copied to that path AFTER the local backup is created
— so losing/corrupting the server's disk doesn't take the only copy of the backup with it.
The path must exist **inside the `web` container** — i.e. it needs to be a directory mounted
into the container (a network drive, `rclone mount`, etc.), not an arbitrary path on the
host. Copying goes through `rsync` (if the binary is present in the image) or falls back to
`shutil.copy2` (the actual default today, since `rsync` isn't installed into the image yet);
see `copy_backup_offsite()` in `backend/helpers.py`. A failure during the offsite copy
doesn't affect the local backup or rotation — it's only logged.

Example of mounting an rclone remote as a directory:

```bash
# on the host: mount the rclone remote into a directory
rclone mount myremote:vrising-backups /opt/vrising-backups-mount --daemon

# in docker-compose.yml, the web service — add a volume and env var:
#   volumes:
#     - /opt/vrising-backups-mount:/mnt/offsite-backups
#   environment:
#     - BACKUP_REMOTE_PATH=/mnt/offsite-backups
```
The same approach works for a regular network share (NFS/SMB) mounted on the host and passed
through to the container the same way.

---

## Staging environment

`docker-compose.staging.yml` brings up the same stack (same image/code) as a separate compose
project, with separate container names, a separate port, and a separate DB volume — it can run
**alongside** the production stack on the same server without touching it, to verify changes
before running `sudo js`.

```bash
cp .env .env.staging               # or .env.example if you don't have a .env yet
# edit .env.staging if needed (e.g. STAGING_HTTP_PORT)

docker compose -f docker-compose.staging.yml --env-file .env.staging up -d --build

# the site is available at http://<server-IP>:8080 (port configurable via STAGING_HTTP_PORT)

# logs / stopping it
docker compose -f docker-compose.staging.yml logs -f
docker compose -f docker-compose.staging.yml down          # add -v to wipe the staging DB
```

What's deliberately different from production (see the comments in the file itself):
- its own SQLite DB (`vrising-staging_db_data`, never overlaps with the production volume);
- `/var/run/docker.sock` isn't mounted — staging shouldn't have control over the host's Docker
  (SSL setup/self-update from the admin panel aren't available in staging);
- no `umami`/`umami-db` services and no SSL — just plain HTTP on a separate port
  (`nginx/nginx.staging.conf` — a copy of `nginx.conf` without the `/analytics/` block).
