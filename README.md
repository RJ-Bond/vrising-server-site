...
### Статус серверов (`status.html`)
- Упрощённая публичная витрина статуса: онлайн/офлайн каждого сервера и аптайм за 7 и 30 дней, без входа на сайт — быстрый ответ на «сервер сейчас работает?» без деталей `servers.html`
...
- Просмотр рейтинга на произвольную дату в прошлом (`?as_of=YYYY-MM-DD`) — исторический снепшот вместо текущего состояния, без индикатора изменения места (он не имеет смысла в этом режиме)
...
- Сравнение кланов: выбор до 2 кланов чекбоксами и модальное окно бок о бок (общая/средняя мощь, число участников, онлайн сейчас)
...
- Категории товаров с фильтром по каталогу
- Вишлист — сохранить понравившийся товар в избранное, не покупая сразу; отдельная лента «Моё избранное»
- Отдельным товарам может быть задан недельный лимит покупок на игрока (`weekly_limit_per_user`) — остаток лимита виден в карточке товара
...
- Двухфакторная аутентификация (2FA/TOTP) — включение/отключение в профиле, одноразовые резервные коды на случай потери устройства с 2FA (можно перевыпустить партию — старые коды, включая ещё не использованные, при этом теряют силу)
- Принудительный разлогин со всех остальных устройств одной кнопкой в профиле (`POST /api/auth/logout-everywhere`) — сессия, из которой нажата кнопка, остаётся активной
- Самостоятельное безвозвратное удаление аккаунта из профиля, с подтверждением паролем
